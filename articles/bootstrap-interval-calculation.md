...
- **`bootstrap_results`**: A dataframe containing the bootstrap
...
- **`aggregate`**: Logical. If `TRUE` (default), the function returns
  ...
  added to the original bootstrap dataframe `bootstrap_results`.
...
- **`fun`**: Only used when `type = "bca"` and no boot method is used. A
  ...
  `data_cube$data`. This function should return a dataframe containing a
  column named `diversity_val` with the statistic to be evaluated.
...
time.](bootstrap-interval-calculation_files/figure-html/unnamed-chunk-15-1.png)
...
(ref).](bootstrap-interval-calculation_files/figure-html/unnamed-chunk-21-1.png)
...
time.](bootstrap-interval-calculation_files/figure-html/unnamed-chunk-29-1.png)
...
![Confidence intervals for evenness over
time.](bootstrap-interval-calculation_files/figure-html/unnamed-chunk-33-1.png)
...
time.](bootstrap-interval-calculation_files/figure-html/unnamed-chunk-40-1.png)
...
unique species, is negatively biased in bootstrap replicates. This leads
to an extreme mismatch between the original estimate and the
...
![Confidence intervals for richness over
time.](bootstrap-interval-calculation_files/figure-html/unnamed-chunk-43-1.png)
