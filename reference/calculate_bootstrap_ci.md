...
- bootstrap_results:
  ...
  A dataframe with bootstrap replicates, or a `boot` object, or a list
  of `boot` objects. For dataframes, each row is a bootstrap replicate
  and must include columns `rep_boot`, `est_original`, and grouping
  variables. For `boot` objects, confidence intervals are either
  computed directly using
  [`boot::boot.ci()`](https://rdrr.io/pkg/boot/man/boot.ci.html) or the
  objects are converted to a dataframe, depending on the value of
  `no_bias`.
...
- aggregate:
  ...
  original bootstrap dataframe `bootstrap_results`.
...
- `conf`: The confidence level of the interval When `aggregate = FALSE`,
  the dataframe contains the columns from `bootstrap_results` with one
  row per bootstrap replicate.
