...
**dubicube** can delegate the bootstrap resampling procedure to the
well-established **boot** package (Canty et al.,
[2025](https://doi.org/10.32614/CRAN.package.boot)). By default
(`method = "smart"`), dubicube will automatically choose the
bootstrapping method which is best for the given configuration. However
the user can specify a specific method manually with
`method = "boot_whole_cube"`, or `method = "boot_group_specific"`. In
these cases,
[`bootstrap_cube()`](https://b-cubed-eu.github.io/dubicube/reference/bootstrap_cube.md)
...
The differences in performance and the smart selection of the
appropriate method are discussed below.
...
is used or the **dubicube** internal code. The label “boot” indicates
that **dubicube** relies on the **boot** package for resampling and
interval calculation, whereas “no boot” denotes that the same task is
performed using **dubicube**’s internal code paths.
...
![Comparison of execution time between bootstrap
methods.](whole-cube-versus-group-specific-bootstrap_files/figure-html/unnamed-chunk-15-1.png)
