...
bootstrap_results %>%
  ...
  distinct(year, est_original, se_boot_log) %>%
  # Visualise
  ggplot(aes(x = year, y = est_original)) +
  effectclass::stat_fan(aes(link_sd = se_boot_log),
                        link = "log", max_prob = 0.95, fill = "cornflowerblue") +
  geom_errorbar(data = ci_mean_obs_lognorm,
              aes(ymin = ll, ymax = ul),
              position = position_dodge(0.8), linewidth = 0.6) +
  geom_point(data = estimate_mean_obs, aes(y = value, shape = Legend),
...
However, we see that the log-normal bootstrap intervals do not agree with the fans.
This is because the fan plot does not account for bias.
...
# Optional: define alpha levels for each conf level manually
alpha_levels2 <- c(
  "0.95" = 0.1,
  "0.85" = 0.2,
  "0.75"  = 0.3,
...
ggplot(out_ci, aes(x = year)) +
    geom_ribbon(
        aes(ymin = ll, ymax = ul, fill = conf_chr, alpha = conf_chr),
        color = NA
    ) +
  geom_errorbar(data = ci_mean_obs_lognorm,
              aes(ymin = ll, ymax = ul),
  ...
  geom_point(data = estimate_mean_obs, aes(y = value, shape = Legend),
    ...
    scale_fill_manual(values = rep("blue", length(alpha_levels)), guide = "none") +
    theme_minimal() +
    labs(
...
bootstrap_results %>%
  # Calculate standard error on link scale
  mutate(rep_boot_log = log(rep_boot),
         est_boot_log = mean(rep_boot_log),
         se_boot_log = sd(rep_boot_log),
         .by = "year") %>%
  # Get unique estimates per year
  ...
  # Visualise
  ggplot(aes(x = year)) +
  effectclass::stat_fan(aes(y = est_original / bias_log, link_sd = se_boot_log),
...
We can only use this function if we assume no bias and account for it in the plot:

```{r}
ci_mean_obs_lognorm2 <- calculate_bootstrap_ci(
  bootstrap_samples_df = bootstrap_results,
  grouping_var = "year",
  type = c("norm"),
  conf = 0.95,
...
bootstrap_results %>%
  # No bias
  dplyr::rowwise() %>%
  dplyr::mutate(rep_boot = rep_boot - bias_boot) %>%
  dplyr::ungroup() %>%
  # Calculate standard error on link scale
  mutate(rep_boot_log = log(rep_boot),
         est_boot_log = mean(rep_boot_log),
  ...
  # Visualise
  ggplot(aes(x = year)) +
  effectclass::stat_fan(aes(y = est_original, link_sd = se_boot_log),
                        link = "log", fill = "cornflowerblue", max_prob = 0.95) +
  geom_errorbar(data = ci_mean_obs_lognorm2,
              aes(ymin = ll, ymax = ul),
              position = position_dodge(0.8), linewidth = 0.6) +
  geom_point(data = estimate_mean_obs, aes(y = value, shape = Legend),
             colour = "firebrick", size = 2) +
