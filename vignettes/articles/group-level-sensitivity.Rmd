...
These values are not very informative on their own.
Are these estimates based on reliable data?
How certain are we about these estimates?

To inform us about the first question, we perform leave-one-species-out cross-validation to investigate the impact of included species on the calculation of these values.

### Leave-one-species-out cross-validation

For this, we rely on the `cross_validate_cube()` function. It relies on the following arguments:

- **`data_cube`**:  
  The input data, either as a processed data cube (from `b3gbi::process_cube()` or `b3gbi::simulate_cube()`), or simply the dataframe inside it (i.e. `processed_cube$data`). For faster computation, passing just the dataframe is recommended.
...
We see that it becomes larger for later years.
...
If we look at the error values themselves, we see indeed more outliers in later years.
A species key 2481139 in year 2015 with error -4 means that without species 2481139, the average number of occurrences would be 4 lower than the estimate based on the full dataset (in this case around 51 instead of 55).

```{r}
# Get original estimates (full dataset)
original_estimates <- cv_results %>%
...
This means that these species are probably overrepresented in the cube and increase the estimates for our statistic unrelated to the true underlying diversity signal. Their disproportionate contribution may skew the results, leading to inflated values in years where those species were heavily sampled. This highlights how group-level sensitivity analysis can reveal data biases introduced by uneven sampling or targeted monitoring efforts.
