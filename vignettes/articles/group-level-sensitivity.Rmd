...
cv_results_path <- file.path(cache_path, "cv_results.Rds")
...
if (file.exists(cv_results_path)) {
  ...
  saveRDS(cv_results, cv_results_path)
}
```
...
As stated in the documentation, it is also possible to cross-validate over a processed cube generated by `b3gbi::process_cube()` with an indicator function of that package (e.g. evenness per year with `b3gbi::pielou_evenness_ts()`).
However, this approach increases computation time.

```r
cv_results_evenness <- cross_validate_cube(
