...
ci_mean_whole <- calculate_bootstrap_ci(
  bootstrap_results = bootstrap_mean_whole,
  grouping_var = "year",
...
ci_mean_group <- calculate_bootstrap_ci(
  bootstrap_results = bootstrap_mean_group,
  grouping_var = "year",
  type = c("perc", "bca", "norm", "basic"),
  conf = 0.95,
  data_cube = processed_cube,   # Required for BCa
...
ci_mean_boot_whole <- calculate_bootstrap_ci(
  bootstrap_results = bootstrap_mean_boot_whole,
  grouping_var = "year",
  type = c("perc", "bca", "norm", "basic"),
  conf = 0.95
)
ci_mean_boot_group <- calculate_bootstrap_ci(
  bootstrap_results = bootstrap_mean_boot_group,
  grouping_var = "year",
  type = c("perc", "bca", "norm", "basic"),
  conf = 0.95
)
...
if (
  system.file(
    "bootstrapping", "speed_comparison.rds", package = "dubicube"
  ) == ""
) {
  ...
  saveRDS(
    total_df,
    file.path("..", "..", "inst", "bootstrapping", "speed_comparison.rds")
  )
} else {
  total_df <- readRDS(
    system.file("bootstrapping", "speed_comparison.rds", package = "dubicube")
  )
}
```
...
```{r, echo=FALSE, warning=FALSE, fig.height=6}
#| fig.alt: >
#|   Comparison of execution time between bootstrap methods.
# Visualisation
total_df %>%
  mutate(boot = factor(boot, levels = c("no boot", "boot"))) %>%
  ggplot(aes(x = cat, y = time_ms, colour = boot)) +
