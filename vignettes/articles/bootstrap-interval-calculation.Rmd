...
> **Note:** The BCa interval (`type = "bca"`) is not computed in this tutorial due to its high computational cost. Consequently, the `data_cube` and `fun` arguments are not required for the interval types used here (`"perc"`, `"norm"`, `"basic"`) and will not have any effect on these interval calculations. However, we include them in the example to illustrate their usage in case BCa intervals are desired. We are actively working on optimizing BCa computations for improved performance in future versions.
...
```{r, echo=FALSE, message=FALSE}
bootstrap_results_ref_path <- file.path(cache_path, "bootstrap_results_ref.Rds")
if (file.exists(bootstrap_results_ref_path)) {
  bootstrap_results_ref <- readRDS(bootstrap_results_ref_path)
} else {
  ...
  saveRDS(bootstrap_results_ref, bootstrap_results_ref_path)
}
```
...
```{r, echo=FALSE, message=FALSE}
ci_mean_obs_ref_path <- file.path(cache_path, "ci_mean_obs_ref.Rds")
if (file.exists(ci_mean_obs_ref_path)) {
  ci_mean_obs_ref <- readRDS(ci_mean_obs_ref_path)
} else {
  ...
  saveRDS(ci_mean_obs_ref, ci_mean_obs_ref_path)
}
```
...
We see that the mean number of observations is much lower in later years compared to 2011.
Because the intervals are lower than zero from 2014 onwards, we might even say it is significant.
...
bird_cube_belgium_extreme <- bird_cube_belgium[rows, ]

# Make dataset even
bird_cube_belgium_extreme$n <- rnbinom(nrow(bird_cube_belgium_extreme),
                                       size = 2, mu = 100)

# Process cube
processed_cube_small <- process_cube(
  bird_cube_belgium_extreme,
  first_year = 2011,
  last_year = 2015,
  cols_occurrences = "n"
)
...
calc_evenness <- function(data) {
  year <- scientificName <- obs <- tot <- p_ln_p <- ln_S <- NULL
...
We perform bootstrapping as before. Note that you can also perform bootstrapping of `processed_cube_small` using the **b3gbi** function `pielou_evenness_ts()`.

```{r}
bootstrap_results_evenness <- bootstrap_cube(
  data_cube = processed_cube_small$data,
  fun = calc_evenness,
  grouping_var = "year",
  samples = 1000,
  seed = 123
...
We get a warning message because we now have a relatively small dataset.
...
For values between 0 and 1, we can use the logit and expit functions:
...
We enter them via `calculate_bootstrap_ci()`.
