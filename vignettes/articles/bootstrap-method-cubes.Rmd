...
#| fig.alt: >
#|   Bootstrap resampling a data cube in tabular format.
knitr::include_graphics("figures/bootstrapping.png")
```
...
The distributions are symmetric indicating a low bootstrap bias (true estimate and bootstrap estimate are close to each other), which can be expected for a mean.
...
As stated in the documentation, it is also possible to bootstrap a processed cube generated by `b3gbi::process_cube()` with an indicator function of that package (e.g. evenness per year with `b3gbi::pielou_evenness_ts()`).
...
Because the full bootstrap distribution is lower than zero, we might even say it is significantly lower.
